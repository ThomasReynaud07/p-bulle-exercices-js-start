## Feedback à 80%

- (Rythme de travail) Traces d'activité: OK
- (Maîtrise technique): Niveau = 47, Note au premier test = 3,7 -> insuffisant à ce stade
- (Autonomie) Jokers: OK
- (Processus de travail) jdt: OK, à un détail près :
  - le 28 novembre, vous notez deux heures et 50 minutes pour un exercice. C'est énorme. Qu'est-ce qui explique une pareille durée ? La réponse à cette question devrait se trouver dans la colonne description de votre journal.
- (durabilité) .gitignore: OK

- Globalement:
  - Il ne vous reste qu'un petit effort à fournir lors de la dernière séance pour assurer le minimum
  - Mais je trouverais dommage que vous vous contentiez du minimum. J'ai fait la même remarque à Ryan avec qui vous me sembler vous distraire: vous êtes capable de plus. J'espère que 2026 sera l'année où vous réaliserez qu'il est dans votre intérêt d'adopter une attitude plus active.


## Final

- Voir grille